#=============================================================
# Title: Surface processing pipeline
# Author: Andrea Gondova (modified by Sungmin You) 
...
def main(args):
	...
	# 	os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/relabel_segmentation.py --iSEGM {} --oSEGM {}'.format(iSEGM, os.path.join(outdir, 'cp_manual_seg_5.nii')))
	...
	print('Surface registration and measurements')
	case_path = os.path.dirname(final_outdir)	# case_path is ${file}

	os.system('adapt_object_mesh {0}/surfaces/lh.wm_81920.obj {0}/surfaces/lh.smoothwm.to31_81920.obj 0 100 0 0'.format(case_path))
	os.system('adapt_object_mesh {0}/surfaces/rh.wm_81920.obj {0}/surfaces/rh.smoothwm.to31_81920.obj 0 100 0 0'.format(case_path))

	os.system('/neuro/users/mri.team/Codes/pipeline_2024/part1/src/obj2asc {0}/surfaces/lh.smoothwm.to31_81920.obj {0}/surfaces/lh.smoothwm.to31_81920.asc'.format(case_path))
	os.system('/neuro/users/mri.team/Codes/pipeline_2024/part1/src/obj2asc {0}/surfaces/rh.smoothwm.to31_81920.obj {0}/surfaces/rh.smoothwm.to31_81920.asc'.format(case_path))

	os.system('adapt_object_mesh {0}/surfaces/lh.pial_81920.obj {0}/surfaces/lh.pial.to31_81920.obj 0 100 0 0'.format(case_path))
	os.system('adapt_object_mesh {0}/surfaces/rh.pial_81920.obj {0}/surfaces/rh.pial.to31_81920.obj 0 100 0 0'.format(case_path))

	os.system('/neuro/users/mri.team/Codes/pipeline_2024/part1/src/obj2asc {0}/surfaces/lh.pial.to31_81920.obj {0}/surfaces/lh.pial.to31_81920.asc'.format(case_path))
	os.system('/neuro/users/mri.team/Codes/pipeline_2024/part1/src/obj2asc {0}/surfaces/rh.pial.to31_81920.obj {0}/surfaces/rh.pial.to31_81920.asc'.format(case_path))

	#os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/Surface_measures_original_v0.0.py --input_fol {}'.format(case_path))
	os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/Surface_measures_resampled_v0.0.py --input_fol {}'.format(case_path))
...
def _extract_wm_mesh(segmentation, output_wm, hemi, taubin, subsampling, convert='yes'):
	...
	os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/1_Inner_CP_surface_v0.0.py \
			--input_seg={} \
			--output_surface={} \
			--label={} \
			--side={} \
	...
	'''
	os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/smtherr.py \
			--input_surface={} \
	...
	os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/surfdisterr.py \
			--input_surface={} \
			--input_mask={} \
			--label={} \
...
def _skeletonization(outdir, segmentation, lz_value, n_iteration):
	...
	os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/2_CSF_skeletonization_iteration_v0.0.py \
			--dir={} \
			--input_seg={} \
			--lz_value={} \
			--n_iteration={} \
...
def _laplacian(outdir, hemi, CLASP_CP_label, convert='yes' ):
	...
	print(iCLASP)
	os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/3_Laplacian_field_v0.0.py \
			--input_seg={} \
			--output_laplacian={} \
			--CLASP_label={} \
...
def _extract_pial_mesh(outdir, hemi, stretch_wgh, laplacian_weight, convert='yes'):
	...
	os.system('python3 /neuro/users/mri.team/Codes/pipeline_2024/part2/src/4_Outer_surface_extraction_v0.0.py \
			--laplacian_map={} \
			--inner_surface={} \
			--outer_surface={} \
			--stretch_weight={} \
	...
	if convert == 'yes':
		os.system('/neuro/users/mri.team/Codes/pipeline_2024/part2/src/convert_obj2gii.sh {}/{}.pial_81920.obj'.format(outdir, hemi))
