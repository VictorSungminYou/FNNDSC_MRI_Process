...
#alignment  ++ add subparser flags for alignment correction: "-cost mutualinfo -searchcost mutualinfo" and "-searchcost normmi -cost normmi"
def align():
    ...
    def alignment_func(temp_recon_a):
        ...
        for t in a_temp:
            ...
            #if subparser flag given (normni or mutualinfo):
                #do above line but with the additional flag option
                ## os.system('flirt -in /neuro/labs/grantlab/research/HyukJin_MRI/templates_for_alginment/template-'+t+'/template-'+t+'.nii -ref '+temp_recon_a+'/recon.nii \
                ##-out '+temp_recon_a+'/Temp-Recon-7dof-'+t+'.nii -omat '+temp_recon_a+'/Temp-Recon-7dof-'+t+'.xfm -dof 7  '+correction_flag+' -searchrx -180 180 -searchry -180 180 -searchrz -180 180;')
        ...
        os.system('convert_xfm -omat '+temp_recon_a+'/recon_to31.xfm -concat /neuro/users/mri.team/fetal_mri/templates_for_alginment/original//template-'+temp+'/template-'+temp+'to31.xfm '+temp_recon_a+'/InvAligned-'+temp+'.xfm')
...
#optional segmentation if the automatic segmentation with the attention model did not produce great results
def auto_seg_No_Att():
